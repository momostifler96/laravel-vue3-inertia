        ...
        <div class="md:flex w-full">
            ...
            <div class="w-full md:w-1/2 py-10 px-5 md:px-10">
                <div class="text-center mb-10">
                    ...
                    <p>Entrez vos invormation de connexion</p>
                </div>
                <form @submit.prevent="submit">

                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-12">
                            <label for="" class="text-xs font-semibold px-1">Identifiant</label>
                            <div class="flex">
                                <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-lock-outline text-gray-400 text-lg"></i></div>
                                <input type="text" v-model="form.uuid" v-maska="" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="Momoledev" required>
                            </div>
                        </div>
                    </div>
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-12">
                            ...
                            <div class="flex">
                                <div class="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center"><i class="mdi mdi-lock-outline text-gray-400 text-lg"></i></div>
                                <input type="password" v-model="form.password" v-maska="" class="w-full -ml-10 pl-10 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-indigo-500" placeholder="************" required>
                            </div>
                        </div>
                    </div>
                    <div class="flex -mx-3 my-3 px-3">
                         <label class="form-check-label flex items-center">
                            <input type="checkbox" @change="()=>{form.remember=!form.remember}" class="form-check-input mr-2" name="" id="" value="checkedValue" checked>
                            Rester connecter 
                          </label>
                    </div>
                    <div class="flex -mx-3">
                        <div class="w-full px-3 mb-5">
                            <button type="submit" class="block w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">SE CONNECTER</button>
                        </div>
                    </div>
                    <div class="flex justify-center mt-3"><Link :href="route('register')" class="font-semibold">S'inscrire</Link></div>
                </form>
            </div>
        </div>
    </div>
...
const form = ref({
 ...
 function submit() {
      Inertia.post(route("login.store"), form.value)
    }
</script>

<style scoped>@import url('https://cdnjs.cloudflare.com/ajax/libs/MaterialDesign-Webfont/5.3.45/css/materialdesignicons.min.css')</style>
